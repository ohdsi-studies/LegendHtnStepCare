  ...
  for (i in 1:nrow(cohortDefinition)){
    ...
    for(j in 1:length(cohortDefinitionJson$ConceptSets$expression$items)){
      ...
      cohortDefinitionJson$ConceptSets$expression$items[[j]]$concept = merge(cohortDefinitionJson$ConceptSets$expression$items[[j]]$concept %>% select(CONCEPT_ID) %>% mutate(rowId = row_number()),
                                                                             conceptName, by = 'CONCEPT_ID', all.x = T) %>% arrange(rowId) %>% select(-rowId)
    }
...
createCohortDefinitionAgeSubgroup1 <- function(cs1, cs2){
  ...
  cd <- cohort(
    entry = entry(drugExposure(cs_second, age(bt(18, 44)), firstOccurrence()),
                  observationWindow = continuousObservation(priorDays = 365),
                  qualifiedLimit = 'First'),
    attrition = attrition(
      withAll(atLeast(1, query = drugExposure(cs_first), duringInterval(eventStarts(-Inf, -30, index = 'startDate')))),
...
createCohortDefinitionAgeSubgroup2 <- function(cs1, cs2){
  ...
  cd <- cohort(
    entry = entry(drugExposure(cs_second, age(bt(45, 64)), firstOccurrence()),
                  observationWindow = continuousObservation(priorDays = 365),
                  qualifiedLimit = 'First'),
    attrition = attrition(
      withAll(atLeast(1, query = drugExposure(cs_first), duringInterval(eventStarts(-Inf, -30, index = 'startDate')))),
...
createCohortDefinitionAgeSubgroup3 <- function(cs1, cs2){
  ...
  cd <- cohort(
    entry = entry(drugExposure(cs_second, age(gte(65)), firstOccurrence()),
                  observationWindow = continuousObservation(priorDays = 365),
                  qualifiedLimit = 'First'),
    attrition = attrition(
      withAll(atLeast(1, query = drugExposure(cs_first), duringInterval(eventStarts(-Inf, -30, index = 'startDate')))),
...
createCohortDefinitionMaleSubgroup <- function(cs1, cs2){
  ...
  cd <- cohort(
    entry = entry(drugExposure(cs_second, age(gte(18)), male(), firstOccurrence()),
                  observationWindow = continuousObservation(priorDays = 365),
                  qualifiedLimit = 'First'),
    attrition = attrition(
      withAll(atLeast(1, query = drugExposure(cs_first), duringInterval(eventStarts(-Inf, -30, index = 'startDate')))),
...
createCohortDefinitionFemaleSubgroup <- function(cs1, cs2){
  ...
  cd <- cohort(
    entry = entry(drugExposure(cs_second, age(gte(18)), female(), firstOccurrence()),
                  observationWindow = continuousObservation(priorDays = 365),
                  qualifiedLimit = 'First'),
    attrition = attrition(
      withAll(atLeast(1, query = drugExposure(cs_first), duringInterval(eventStarts(-Inf, -30, index = 'startDate')))),
...
createCohortDefinitionWhiteSubgroup <- function(cs1, cs2){
...
createCohortDefinitionBlackSubgroup <- function(cs1, cs2){
...
createCohortDefinitionHispanicSubgroup <- function(cs1, cs2){
...
createCohortDefinitionSubgroup = function(connection, cdm_database_schema, idConvention = 1000000, subgroupName, subgroupFunction = createCohortDefinitionAgeSubgroup1){
  primary = c('ACEI', 'ARB', 'ndCCB', 'dCCB', 'Thiazide')
  secondary = c('ACEI', 'ARB', 'BB', 'ndCCB', 'dCCB', 'Thiazide')
  cohortDefinitionSubgroups = primary %>% merge(secondary, all = T) %>% filter(x != y) %>% mutate(combi = paste(y, 'after', x))
  cohortDefinitionSubgroups = cohortDefinitionSubgroups %>% filter(combi != 'ACEI after ARB' & combi != 'ARB after ACEI') %>% arrange(combi) %>% mutate(combi = paste(combi, subgroupName))
  cohortDefinitionSubgroups$cohort_id = seq(1:nrow(cohortDefinitionSubgroups)) + idConvention
  cohortDefinitionSubgroups$json = NA
  cohortDefinitionSubgroups$sql = NA
  cohortDefinitionSubgroups = cohortDefinitionSubgroups %>% select(cohort_id, x, y, combi, json, sql)
  colnames(cohortDefinitionSubgroups) = c('cohortId', 'First', 'Second', 'Combi', 'json', 'sql')
  
  for(i in 1:nrow(cohortDefinitionSubgroups)){
    result = subgroupFunction(cs1 = conceptDefinition[which(names(conceptDefinition) == cohortDefinitionSubgroups$First[i])][[1]],
                                                cs2 = conceptDefinition[which(names(conceptDefinition) == cohortDefinitionSubgroups$Second[i])][[1]])
    cohortDefinitionSubgroups$json[i] = result$json
    cohortDefinitionSubgroups$sql[i] = result$sql
    rm(result)
  }
  
  cohortDefinitionSubgroups = cohortDefinitionSubgroups %>% select(-First, -Second)
  colnames(cohortDefinitionSubgroups) = c('cohortId', 'cohortName', 'json', 'sql')
  
  for (i in 1:nrow(cohortDefinitionSubgroups)){
    cohortDefinitionJson = cohortDefinitionSubgroups$json[i] %>% jsonlite::fromJSON()
    for(j in 1:length(cohortDefinitionJson$ConceptSets$expression$items)){
      sql = 'select concept_id, concept_name, concept_code, domain_id, vocabulary_id, concept_class_id, standard_concept, invalid_reason from @vocabulary_database_schema.concept where concept_id in (@concept_ids)'
      sql = SqlRender::render(sql, 
                              vocabulary_database_schema = cdm_database_schema,
    ...
    cohortDefinitionSubgroups$json[i] = jsonlite::toJSON(cohortDefinitionJson, pretty = T, auto_unbox = T)
  }
  
  return(cohortDefinitionSubgroups)
}

cohortDefinitionAgeSubgroup1 = createCohortDefinitionSubgroup(connection, cdm_database_schema, idConvention = 1100000, subgroupName = 'age group 1', subgroupFunction = createCohortDefinitionAgeSubgroup1)
cohortDefinitionAgeSubgroup2 = createCohortDefinitionSubgroup(connection, cdm_database_schema, idConvention = 1200000, subgroupName = 'age group 2', subgroupFunction = createCohortDefinitionAgeSubgroup2)
cohortDefinitionAgeSubgroup3 = createCohortDefinitionSubgroup(connection, cdm_database_schema, idConvention = 1300000, subgroupName = 'age group 3', subgroupFunction = createCohortDefinitionAgeSubgroup3)
cohortDefinitionMaleSubgroup = createCohortDefinitionSubgroup(connection, cdm_database_schema, idConvention = 1400000, subgroupName = 'male group', subgroupFunction = createCohortDefinitionMaleSubgroup)
cohortDefinitionFemaleSubgroup = createCohortDefinitionSubgroup(connection, cdm_database_schema, idConvention = 1500000, subgroupName = 'female group', subgroupFunction = createCohortDefinitionFemaleSubgroup)
cohortDefinitionWhiteSubgroup = createCohortDefinitionSubgroup(connection, cdm_database_schema, idConvention = 1600000, subgroupName = 'white group', subgroupFunction = createCohortDefinitionWhiteSubgroup)
cohortDefinitionBlackSubgroup = createCohortDefinitionSubgroup(connection, cdm_database_schema, idConvention = 1700000, subgroupName = 'black group', subgroupFunction = createCohortDefinitionBlackSubgroup)
cohortDefinitionHispanicSubgroup = createCohortDefinitionSubgroup(connection, cdm_database_schema, idConvention = 1800000, subgroupName = 'hispanic group', subgroupFunction = createCohortDefinitionHispanicSubgroup)

cohortDefinitionFinal = rbind(cohortDefinition,
